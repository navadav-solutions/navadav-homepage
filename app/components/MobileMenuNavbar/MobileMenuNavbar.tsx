...
const MobileMenuNavbar = ({ handleCloseMenu, isAnimating, setIsMobileServicesOpen, isMobileServicesOpen, pathname }: MobileMenuNavbarProps) => {
  return (
    <>
    {/* Overlay con animación de fade */}
    <div
      className={`fixed inset-0 bg-black/50 z-40 lg:hidden transition-opacity duration-300 ease-in-out ${isAnimating ? "opacity-100" : "opacity-0"
        }`}
      onClick={handleCloseMenu}
    />
    {/* Panel del menú con animación de slide */}
    <div
      className={`fixed top-0 right-0 h-full w-[80%] max-w-sm bg-white shadow-xl z-50 lg:hidden transform transition-transform duration-300 ease-out ${isAnimating ? "translate-x-0" : "translate-x-full"
        }`}
    >
      <div className="flex flex-col p-6 h-full">
        <div className="flex justify-between items-center mb-8">
          <Image
            ...
            width={100}
            height={80}
            className={`w-35 h-12 transition-opacity duration-300 delay-100 ${isAnimating ? "opacity-100" : "opacity-0"
              }`}
          />
          <button
            onClick={handleCloseMenu}
            className={`text-gray-600 hover:text-gray-800 text-2xl transition-all duration-200 hover:rotate-90 ${isAnimating ? "opacity-100 scale-100" : "opacity-0 scale-0"
              }`}
          >
            ×
          </button>
        ...
        <nav className="flex flex-col gap-4">
          ...
          <div
            className={`border-t border-gray-200 my-4 transition-all duration-300 ${isAnimating
